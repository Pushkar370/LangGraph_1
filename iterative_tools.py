...
def writer_node(state: State) -> dict:
    ...
    1. A genuinely new attempt (first draft, or a rewrite after rejection) —
       builds a fresh prompt and increments `attempt`.
    2. Continuing right after a tool call returned results — reuses the
       existing message history (which already includes the tool output)
       instead of discarding it, and does NOT increment `attempt`.
    """
    messages_so_far = state["messages"]

    # Case 2: last message is a tool result -> just let the model finish
    # writing using what it already has in context.
    if messages_so_far and getattr(messages_so_far[-1], "type", None) == "tool":
        response = writer_llm_with_tools.invoke(messages_so_far)
        return {"messages": [response]}

    # Case 1: a new attempt.
...
def extract_draft_node(state: State) -> dict:
    ...
    draft = content if isinstance(content, str) else content[0]['text']
    print(f"\n\n generated post \n {draft} \n ")
    return {"draft": draft}
...
# KEY FIX: after tools run, go back to the writer so it can actually use
# the search results to produce the final draft, instead of skipping
# straight to the reviewer with an empty/stale draft.
